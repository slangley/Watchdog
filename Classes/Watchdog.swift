...
@objc public class Watchdog: NSObject {
     ...
     */
    public init(threshold: Double = 0.4, strictMode: Bool = false, handler:(() -> ())? = nil) {
        ...
        if let pingThreadHandler = handler {
            self.pingThread = PingThread(threshold: threshold, handler:pingThreadHandler)
        } else {
            self.pingThread = PingThread(threshold: threshold, handler:defaultHandler)
        }
